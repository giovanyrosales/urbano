  ...
  <section class="content-header">
      ...
      <div class="container-fluid">
        <div class="card card-gray">
          <div class="card-header">
            <h3 class="card-title">Tabla de Procesos</h3>           
          </div>
                ...
                <div class="row">  
                  <div class="col-sm-12"> 
                    ...
                    <div class="form-group">
                      <label>Nombre</label>
                      <input type="text" class="form-control" id="nombre" maxlength="250" placeholder="Nombre Proceso">
                    </div>                  
                  </div>            
                </div> 
              </div>
   ...
   <div class="modal fade" id="modalEditar">
        <div class="modal-dialog">
              ...
              <div class="card-body">
                <div class="col-sm-12">  
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="eNombre" maxlength="250" placeholder="Nombre Proceso">
                    </div>   
                </div> 
              </div>
            </form>
          ...
          <div class="modal-footer justify-content-between">
            <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
            <button type="button" class="btn btn-primary" onclick="editarProceso()">Guardar</button>
          </div>
 ...
 <!-- incluir tabla --> 
  <script type="text/javascript">	
    $(document).ready(function(){    
      var ruta = "{{ URL::to('admin/tablas/proceso') }}";   
  ...
  function guardarProceso(){
      ...
      if(retorno){
        ...
        axios.post('/admin/proceso/agregar', formData, {  
          })
          .then((response) => {	
  ...
  // mensaje cuando guardamos proceso nuevo *
  function mensajeResponse(valor){
    if(valor.data.success == 1){
      toastr.success('Nuevo proceso creado');
      $('#modalAgregar').modal('hide');             
      var ruta = "{{ URL::to('admin/tablas/proceso') }}";   
      $('#tablaDatatable').load(ruta);
    }else if(valor.data.success == 2){
      toastr.error('Error, Proceso no creado');
    }else{   
      toastr.error('Error de validacion');
    }
  }
    
    // validar antes de agregar proceso *
  ...
  // abre el modal para editar el proceso *
  function abrirModalEditar(id){
    
    spinHandle = loadingOverlay().activate();
    axios.post('/admin/proceso/informacion',{
      'id': id  
        })
        .then((response) => {	
          loadingOverlay().cancel(spinHandle); 
          if(response.data.success = 1){
            ...
            $('#eNombre').val(response.data.proceso.nombre);
            $('#eId').val(response.data.proceso.id);
          }else{ 
            toastr.error('Proceso no encontrado'); 
          }
        })
        .catch((error) => {
          loadingOverlay().cancel(spinHandle); 
  ...
  function editarProceso(){
      ...
      if(retorno){
          ...
          axios.post('/admin/proceso/editar', formData, {        
          })
          .then((response) => {	     
            loadingOverlay().cancel(spinHandle);
  ...
  function mostrarMensajeEditar(valor) {          
      if (valor.data.success == 1) { 
          ...
          var ruta = "{{ URL::to('admin/tablas/proceso') }}";   
          $('#tablaDatatable').load(ruta);   
          toastr.success('Proceso actualizado');          
      }else {
          toastr.error('Error al editar proceso'); 
      }
  }
